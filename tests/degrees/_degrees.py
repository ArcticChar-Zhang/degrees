...
class Degree:
    ...
    def dms(self) -> tuple[int, int, int]:
        """Return tuple(self.deg, self.min, self.sec) with signs(+-)"""
    ...
    def to_complex(self, r: int | float, /) -> complex:
        """This function denotes a complex number with magnitude length and argument self(as theta). Equivalent to\
 self | at_an_angle r."""
        if r < 0:
